...
;	If $ocrTrainChar <> "" Then $ocrOptions = BitOR($ocrOptions, $ocrOptTraining)
   ...
   If $LearnMoreDatabase = "" Then
   $LearnMoreDatabase = @ScriptDir & "\LearnMore.txt"
	  EndIf
...
;				debugStep(@ScriptLineNumber, "$letterManual", $letter, "$data", $data)
				...
				If @error = 1 or @error = 2 Then ;The Cancel button was pushed or Timeout.
					If Not BitAND($ocrOptions, $ocrOptDontSaveErrors) Then
						...
						; Anton's Addition
						FileWriteLine($LearnMoreDatabase, "err" & @TAB & "!" & StringMid($pattern, 2) & $saveWindowForFont & @CRLF)
						FileWriteLine($LearnMoreDatabase, @CRLF)
						FileWrite($LearnMoreDatabase,$image)
					EndIf
					SetError(-2)
